...
from app.desktop.services.audit_logs.audit_logs_service import get_fda_audit_logs, get_national_admin_audit_logs, get_lea_audit_logs, get_system_audit_logs, get_national_admin_system_audit_logs
...
@router.get("/national-admin/system", response_model=AuditLogListResponse)
def list_national_admin_system_audit_logs(
    page: int = Query(1, ge=1),
    limit: int = Query(10, ge=1, le=500),
    action: str | None = None,
    date_from: date | None = None,
    ...
    rows, total = get_national_admin_system_audit_logs(db, page, limit, action, date_from, date_to, search)
